...
CV<-function(x,statistic,trend,type,m=0,M,d=0,tau,p=0,twostep=FALSE){
  ...
  if(type=="cusum"){
    tstat_sim<-rep(NA,M)
    for(l in 1:M){tstat_sim[l]<-cusum(cumsum(rnorm(length(x)))+tre,trend=trend,tau=tau,m=m)}
    crit<-matrix(c(quantile(tstat_sim,c(0.01,0.05,0.1,.99,.95,.9))),ncol=2,nrow=3)
  }
  if(type=="cusum_SK"){
    tstat_sim<-rep(NA,M)
    for(l in 1:M){tstat_sim[l]<-cusum(FI.sim(length(x),1,0,d)+tre,trend=trend,tau=tau,m=m)}
    crit<-matrix(c(quantile(tstat_sim,c(0.01,0.05,0.1,.99,.95,.9))),ncol=2,nrow=3)
  }
  if(type=="MR"){
    tstat_sim<-rep(NA,M)
  ...
  if(type=="LBI")
  {
    ...
    if(statistic=="mean"){
      for(l in 1:M){
        tstat_sim<-LBI(rnorm(length(x))+tre,trend=trend,m=m,tau=tau)
